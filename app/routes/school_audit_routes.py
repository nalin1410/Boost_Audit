...
def start_audit():
    ...
    try:
        ...
        # UPDATE ASSIGNMENT STATUS - Mark school as "in_progress"
        assignment_update_success = update_school_audit_status_in_assignment(
            ...
            audit_id=audit_id,
            assignment_date=localized_dt.strftime('%Y-%m-%d')  # Pass the assignment date
        )

        if not assignment_update_success:
            print(f"⚠️ Warning: Could not update assignment status for {data['school_name']} in {data['city']}")
...
def end_audit():
    ...
    try:
        ...
        # UPDATE ASSIGNMENT STATUS - Mark school as "completed"
        assignment_update_success = update_school_audit_status_in_assignment(
        ...
        if not assignment_update_success:
            print(f"⚠️ Warning: Could not update assignment status for {audit['school_name']} in {audit['city']}")
